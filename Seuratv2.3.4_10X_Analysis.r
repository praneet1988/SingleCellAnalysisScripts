############ This script processes 10X data using Seurat [v2.3.4]###############
...
setwd('Directory_of_interest')
M.data <- Read10X(data.dir = "10X Dir") ######## 10X directory
M <- CreateSeuratObject(raw.data = M.data, min.cells =3, min.genes = 100, project= '10X Data')
##########################################################

############## Filtering, Normalizing and Scaling #########
file <- "File name prefix"                                  #### For example : 10X_E14KI
...
######################### Save Robject ###############################################################
save(M, file = 'Seurat_v2.3.4_10XAnalysis.Robj')
######################################################################################################